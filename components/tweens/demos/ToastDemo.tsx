...
const CODE = `snap(el, { y: 16, opacity: 0, scale: 0.95 })
spring(el, { y: 0, opacity: 1, scale: 1 }, 'snappy')
// after 2s — exit the same way it came in
spring(el, { y: 16, opacity: 0, scale: 0.95 }, 'snappy')`
...
export function ToastDemo() {
  ...
  const animate = () => {
    ...
    if (!el) return
    snap(el, { y: 16, opacity: 0, scale: 0.95 })
    requestAnimationFrame(() => {
      spring(el, { y: 0, opacity: 1, scale: 1 }, 'snappy')
      setTimeout(() => spring(el, { y: 16, opacity: 0, scale: 0.95 }, 'snappy'), 2000)
    })
  }
  return (
